...
if($action == "avatar") {
    ...
    $id = $_REQUEST['id'];
    query_json($db,"SELECT story.id, duo_id, image, story.name, set_id, set_index, text, c.learningLanguage as learningLanguage, c.fromLanguage as fromLanguage FROM story JOIN course c on story.course_id = c.id WHERE story.id = $id");
}
else {
    echo "unknown action";
}
