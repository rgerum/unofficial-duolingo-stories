...
export function replace_with_mapping({text, mapping}, replace, start, end) {
  if(mapping === undefined)
    mapping = init_mapping(text).mapping;
  if(!end)
    end = start;
  ...
  while(match) {
    ...
    if(match.length > 1) {
      ...
      for (let i = 1; i < match.length; i++) {
        ...
        if (p1 !== -1) {
          mapped_text = replace_with_mapping(mapped_text, replacer.substring(0, p1), match.index, match.index);
          replacer = replacer.substring(p1 + 2);
          match.index += p1 + match[i].length;
        }
        else {
          mapped_text = replace_with_mapping(mapped_text, "", match.index, match.index + match[i].length);
          //replacer = replacer.substring(p1 + 2);
      ...
      mapped_text = replace_with_mapping(mapped_text, replace, match.index, match.index + match[0].length);
...
function iter_word_replacements(mapped_text, callback, options) {
  ...
  for(let i = 0; i < mapped_text.text.length+1; i++) {
    ...
    if(tag_start.length === 0 && word) {
      [mapped_text, i] = callback(mapped_text, word, word_start, i, bracket_start.length);
    }
    if(word)
...
export function apply_word_replacements(mapped_text, replacements, options) {
  mapped_text = iter_word_replacements(mapped_text, (mapped_text, word, word_start, i, bracket_level) => {
    if(options.in_brackets !== undefined && options.in_brackets != bracket_level)
    ...
    if(new_word !== undefined) {
      if (new_word.endsWith(":ipa")) {
        new_word = `<phoneme alphabet="ipa" ph="${new_word.substring(0, new_word.length - 4)}">`
        mapped_text = replace_with_mapping(mapped_text, new_word, word_start, word_start);
        i += new_word.length;
        new_word = `</phoneme>`
        mapped_text = replace_with_mapping(mapped_text, new_word, i, i);
        i += new_word.length;
      } else {
        mapped_text = replace_with_mapping(mapped_text, new_word, word_start, word_start + word.length);
...
export function add_word_marks_replacements(mapped_text) {
  mapped_text = iter_word_replacements(mapped_text, (mapped_text, word, word_start, i, bracket_start) => {
...
function apply_fragment_replacements(mapped_text, replacements) {
  mapped_text = iter_word_replacements(mapped_text, (mapped_text, word, word_start, i, bracket_start) => {
    for (let frag in replacements) {
      ...
      mapped_text = replace_with_mapping(mapped_text, new_word, word_start + match.index, word_start + match.index + match[0].length);
  ...
  for (let frag in replacements) {
    mapped_text = find_replace_with_mapping(mapped_text, new RegExp(frag, "i"), replacements[frag]);
  }
  return mapped_text;
}
...
function apply_group(mapped_text, data, options) {
  ...
  for (let section in data) {
    ...
    if (section.toUpperCase() === "LETTERS") {
      mapped_text = apply_letter_replacements(mapped_text, data[section], options);
    }
    if (section.toUpperCase() === "FRAGMENTS") {
      mapped_text = apply_fragment_replacements(mapped_text, data[section], options);
    }
    if (section.toUpperCase() === "WORDS") {
      mapped_text = apply_word_replacements(mapped_text, data[section], options);
    }
  }
  return mapped_text;
}
...
if(0) {
  let speak_text = init_mapping("break this... <break name='xx' />is [be KALO and] Text break");
  ...
  speak_text = apply_fragment_replacements(speak_text, {"eak$": "ion", "^b": "B"})
  console.log(speak_text)
...
if(0) {
  ...
  speak_text = apply_word_replacements(speak_text, {"is": "bla:ipa", "text": "test"})
