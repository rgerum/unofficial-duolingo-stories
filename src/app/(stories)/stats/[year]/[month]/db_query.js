...
export async function get_stats(year, month) {
  let res = await query(
    `SELECT course_id, COUNT(s.course_id) AS count FROM story s
  ...
  let res3 = await query(
    `SELECT s.course_id, COUNT(DISTINCT(sd.user_id)) AS count FROM story_done sd
        JOIN story s on s.id = sd.story_id
         WHERE EXTRACT(MONTH FROM sd.time) = ? AND
         EXTRACT(YEAR FROM sd.time) = ?
    GROUP BY course_id ORDER BY count DESC;`,
  ...
  let res3b = await query(
    `SELECT COUNT(DISTINCT(sd.user_id)) AS count FROM story_done sd
         WHERE EXTRACT(MONTH FROM sd.time) = ? AND
         EXTRACT(YEAR FROM sd.time) = ?
  ...
  let res4 = await query(
    `SELECT s.course_id, COUNT(DISTINCT(sd.story_id)) AS count FROM story_done sd
        JOIN story s on s.id = sd.story_id
         WHERE EXTRACT(MONTH FROM sd.time) = ? AND
         EXTRACT(YEAR FROM sd.time) = ?
    GROUP BY course_id ORDER BY count DESC;`,
  ...
  let res_course = await query(
    `SELECT c.id, c.learningLanguage, c.fromLanguage, c.public FROM course c JOIN language l on l.id = c.learningLanguage ORDER BY l.name;`,
  );
