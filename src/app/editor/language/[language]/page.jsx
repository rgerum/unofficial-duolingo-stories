...
const get_language = cache(async (id) => {
    ...
    if(isNumeric(id)) {
        return [(await query_one_obj(`SELECT * FROM language WHERE id = ? LIMIT 1`, [id])), undefined, undefined];
    }
    else {
        let course = await query_one_obj(`SELECT learningLanguage, fromLanguage, short FROM course WHERE short = ? LIMIT 1`, [id]);
        if(course) {
            id = course.learningLanguage;
            let id2 = course.fromLanguage;
            return [(await query_one_obj(`SELECT * FROM language WHERE id = ? LIMIT 1`, [id])), course, (await query_one_obj(`SELECT * FROM language WHERE id = ? LIMIT 1`, [id2]))];
        }
        return [(await query_one_obj(`SELECT * FROM language WHERE short = ? LIMIT 1`, [id])), undefined, undefined];
    }
})
...
export default async function Page({params}) {
    ...
    // Render data...
    return <>
        <LanguageEditor language={language} language2={language2} speakers={speakers} avatar_names={avatar_names} session={session} course={course}/>
    </>
}
