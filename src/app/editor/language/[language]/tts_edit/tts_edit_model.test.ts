...
test("keeps the selected voice unchanged at the default prosody", () => {
  assert.equal(
    formatTestSpeaker("nl-NL-FennaNeural", 2, 2),
    "nl-NL-FennaNeural",
  );
});
...
test("adds pitch and rate modifiers to pronunciation test voices", () => {
  assert.equal(
    formatTestSpeaker("nl-NL-FennaNeural", 1, 3),
    "nl-NL-FennaNeural(pitch=low, rate=fast)",
  );
  assert.equal(
    formatTestSpeaker("nl-NL-FennaNeural", 4, 2),
    "nl-NL-FennaNeural(pitch=x-high)",
  );
  assert.equal(
    formatTestSpeaker("nl-NL-FennaNeural", 2, 0),
