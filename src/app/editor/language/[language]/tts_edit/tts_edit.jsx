...
export default function Tts_edit({language, language2, speakers, session, course}) {
...
`)
    ...
    async function play(e, id, speaker, name, speakText, mapping, text_clear) {
        ...
        let response2 = await fetch_post(`/audio/create`,
            {"id": 0, "speaker": speaker, "text": speakText.replace("$name", name)});
        ...
        for(let marks of ssml_response.marks) {
            ...
            element.line.content.audio.keypoints.push(
                {audioStart: marks.time, rangeEnd: mapping[last_pos]}
            )
        }
        setElement(element);
        ...
        <Layout language_data={language} language2={language2} session={session} course={course}>
            <div className={styles.root + " " + styles.characterEditorContent}>
                ...
                <div className={styles.avatar_editor + " " + (speakers?.length > 0? "": styles.noVoices)} style={{"overflowY": "scroll"}}>
                    <h2>Input Text</h2>
                    <textarea defaultValue={text} onChange={setText} style={{width: "100%"}}/>
                    <br/>
                    ...
                    <br/>
                    <textarea defaultValue={data} onChange={setData} rows={20} cols={40} style={{width: "100%"}}/>
                </div>
