...
export const get_course_editor = cache(async (course_id) => {
  ...
  const res = await query(
    `SELECT COUNT(sa.id) as approvals, story.id, story.set_id, story.set_index, story.name, story.status, story.image,
...
export async function get_course_import({ course_id, from_id }) {
  ...
  let courses = await query(
    `SELECT s1.id, s1.set_id, s1.set_index, s1.name, image.gilded AS image_done, image.active AS image, COUNT(s2.id) copies
                      FROM story s1
...
ORDER BY count DESC, fromLanguageName;
    `);
  let counts =
    await query_objs(`SELECT c.short, COUNT(DISTINCT(sa.user_id)) as count FROM course c JOIN story s on c.id = s.course_id
    JOIN story_approval sa on s.id = sa.story_id
                                                            WHERE
