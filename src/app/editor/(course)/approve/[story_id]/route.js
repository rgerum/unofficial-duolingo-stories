...
async function set_approve({ story_id, user_id }) {
  let res = await query(
    `SELECT id FROM story_approval WHERE story_id = ? AND user_id = ?;`,
    [story_id, user_id],
  );
  let action;
  if (res.length) {
    await query(
      `DELETE FROM story_approval WHERE story_id = ? AND user_id = ?;`,
      [story_id, user_id],
    );
    action = "deleted";
  } else {
    await query(
      `INSERT INTO story_approval (story_id, user_id) VALUES (?, ?);`,
      [story_id, user_id],
    );
    action = "added";
  }
  let res2 = (
    await query(
      `SELECT COUNT(id) as count FROM story_approval WHERE story_id = ?;`,
      [story_id],
    )
  )[0];
  ...
  let res3 = await query(
    `SELECT story.id, story.public FROM story WHERE set_id = (SELECT set_id FROM story WHERE id = ? LIMIT 1) AND
                                            course_id = (SELECT course_id FROM story WHERE id = ? LIMIT 1) AND status = 'finished' AND deleted = 0;`,
    [story_id, story_id],
  );
  ...
  if (res3.length >= 4) {
    ...
    for (let story of res3) {
      if (story.public === 0) {
        await query(
          `UPDATE story SET public = 1, date_published = ? WHERE id = ?;`,
          [date_published, story.id],
        );
