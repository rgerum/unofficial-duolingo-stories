...
async function set_import({ id, course_id }, { user_id, username }) {
  let data = (
    await query(
      `SELECT duo_id, name, image, set_id, set_index, text, json FROM story WHERE id = ?;`,
      [id],
    )
  )[0];
  ...
  let new_id = await insert("story", data, [
    "duo_id",
    "name",
    "author",
    "image",
  ...
  let data2 = (
    await query(`SELECT text,name,course_id, id FROM story WHERE id = ?;`, [
      new_id,
    ])
  )[0];
