...
export async function login(data) {
  ...
  console.log(reponse);
  if (reponse.status === 403) return false;
  return true;
}
...
export async function register(data) {
  ...
  let response;
  try {
    response = await fetch_post(
      `${backend_user}user.php?action=register`,
      data
    );
  } catch (e) {
    // something wrong :-(
...
export async function activate(data) {
  ...
  let reponse = await fetch_post(
    `${backend_user}user.php?action=activate`,
    data
  );
        ...
        <>
          ...
          {username.role !== 0 ? (
            <button
              ...
              className="button"
              onClick={() => {
                window.location.href = "https://editor.duostories.org";
              }}
            >
              Editor
            </button>
...
export function LoginDialog(props) {
  ...
  async function reset_button() {
    ...
    if (success === false) {
      ...
      setState(2);
      setMessage(
        "Have received an email with a reset link. You may need to look into your spam folder."
      );
    }
  }
      ...
      {showLogin === 1 && username === undefined ? (
        <div id="login_dialog">
          ...
          <div>
            ...
            <p>
              You have to register for the unofficial stories separately, as
              they are an independent project.
            </p>
            <input
              value={usernameInput}
            ...
            <button className="button" onClick={buttonLogin}>
              {state !== 1 ? "Log in" : "..."}
            </button>
            <p>
              Don't have an account?{" "}
              <button className={"link"} onClick={() => setShowLogin(2)}>
                SIGN UP
              </button>
            </p>
            <p>
              Forgot your password?{" "}
              <button className={"link"} onClick={() => setShowLogin(3)}>
                RESET
              </button>
            </p>
          </div>
        ...
        <div id="login_dialog">
          ...
          <div>
            ...
            <p>
              Registration is optional, stories can be accessed even without
              login.
            </p>
            <input
              value={usernameInput}
            ...
            {state === 2 ? (
              ...
              <button className="button" onClick={register_button}>
                {state !== 1 ? "Sign up" : "..."}
              </button>
            )}
            <p>
              Already have an account?{" "}
              <button className={"link"} onClick={() => setShowLogin(1)}>
                LOG IN
              </button>
            </p>
            <p>
        ...
        <div id="login_dialog">
          ...
          <div>
            ...
            <p>
              If you forgot your password, we can send you a link to choose a
              new one.
            </p>
            <input
              value={usernameInput}
            ...
            {state === 2 ? (
              ...
              <button className="button" onClick={reset_button}>
                {state !== 1 ? "Reset" : "..."}
              </button>
            )}
            <p>
              Or still remember your password?{" "}
              <button className={"link"} onClick={() => setShowLogin(1)}>
                LOG IN
              </button>
            </p>
