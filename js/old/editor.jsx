...
function Overview(props) {
    ...
    return <div>
        ...
        <h1 id="title">{ lang ? `Editor - Stories - ${language_data[lang].name} - (from ${language_data[lang_base].name})` : "Editor - Stories"}</h1>
        ...
        : <table id="story_list" style={{display: "inline-block"}} className="js-sort-table js-sort-5 js-sort-desc"
            ...
            {stories.map(story =>
                <tr key={story.id}>
                    ...
                    <td><a href={`editor.html?story=${story.id}&lang=el&lang_base=en`}>{story.name_base}</a></td>
                    ...
                    <td style={{textAlign: "right"}}><span>{story.count}x&nbsp;</span><a
                        href={`story.html?test&story=${story.id}&lang=el&lang_base=en`}>[test]</a></td>
