...
function getInputStringSpeachtext(text) {
    ...
    text = text.replace(/(\.\.\.|â€¦)/g, '<sub alias="">$1</sub><break/>');
    return "<speak>"+text.replace(/([^-|~ ,;.:_?!â€¦]*)\{([^\}:]*):([^\}]*)\}/g, '<phoneme alphabet="$3" ph="$2">$1</phoneme>').replace(/([^-|~ ,;.:_?!â€¦]*)\{([^\}]*)\}/g, '<sub alias="$2">$1</sub>').replace(/~/g, " ").replace(/\|/g, "â€‹")+"</speak>";
}
