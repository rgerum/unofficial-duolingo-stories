...
| CP-05 | Switch the Course filter on `/editor/feedback?status=open` | The shared 60px Feedback header remains mounted while the destination route shows its loading fallback. | [Old shell header during loading](http://richard-aspire-a515-55g.tailed9e74.ts.net:8787/ac58d26c2a25c7c437ab7980/20260821T213026Z-67fb9c25-3a98-48ae-9594-722f500a9a31-89b36484-ecb2-4366-9b19-b3c4471aaab9.png) | Pending authenticated preview capture. | Back remains usable; Feedback icon/title stay visible; no account-only header flash when switching between All and a Course. |
| CP-06 | Open `/editor/course/:course` with throttled route and Convex requests | Search and status filters remain in place while eight compact Story-row skeletons replace both the route fallback and the client-query spinner. | Commit `67f3ab00`: large `Loading` fallback followed by a centered spinner. | Pending authenticated device capture. | No intermediate blank/spinner-only screen; filter geometry does not jump when counts arrive; nested Course routes keep their own loading UI. |
