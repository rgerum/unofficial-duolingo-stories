...
async function phpbb_check_hash(password, hash) {
    return new Promise(function(resolve, reject) {
        exec(`php lib/auth/check_hash.php ${shell_escape(password)} ${shell_escape(hash)}`, (error, stdout, stderr) => {
            if (error) {
                reject(error);
            }
            if (stderr) {
...
async function phpbb_hash(password) {
    return new Promise(function(resolve, reject) {
        exec(`php lib/auth/hash.php ${shell_escape(password)}`, (error, stdout, stderr) => {
            if (error) {
                reject(error);
            }
            if (stderr) {
