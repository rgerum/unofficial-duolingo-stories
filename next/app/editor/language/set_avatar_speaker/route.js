...
export async function set_avatar({id, name, speaker, language_id, avatar_id}) {
    ...
    if(res.length) {
        id = res[0].id;
        return await update("avatar_mapping", {id, name, speaker, language_id, avatar_id}, ["name", "speaker", "language_id", "avatar_id"]);
    }
    return await insert("avatar_mapping", {id, name, speaker, language_id, avatar_id}, ["name", "speaker", "language_id", "avatar_id"]);
}